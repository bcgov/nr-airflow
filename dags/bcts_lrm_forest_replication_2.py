# Second DAG for LRM replication to ODS for BCTS Annual Developed Volume Dashboard
# Existing DAG "bcts-replication-lrm" is causing OOM errors on additional tables due to large data volume.
...
with DAG(
    ...
    catchup=False,
    schedule='45 12 * * MON-FRI',   # 4:45 AM PST
    dag_id=f"bcts-replication-lrm2",
    default_args=default_args,
    description='DAG to replicate second batch of LRM data to ODS for BCTS Annual Developed Volume Dashboard',
) as dag:
    ...
    if ENV == 'LOCAL':
        ...
        run_replication = KubernetesPodOperator(
            ...
            secrets=[lob_secrets, ods_secrets],
            container_resources= client.V1ResourceRequirements(
            requests = {"cpu": "400m", "memory": "4096Mi"},
            limits = {"cpu": "800m", "memory": "6144Mi"}),
