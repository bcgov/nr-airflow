--Step3:Update staus flag to 'C' for closed records
Update lob_dq_replication_hist.ats_connectivity_hist a 
SET record_active_ind='C'
from (
select d.application_id application_id
FROM ( select * from lob_dq_replication_hist.ats_connectivity_hist where effective_end_dttm=CAST('9999-12-31 00:00:00' as timestamp(0)))d 
	left 	join  lob_dq_replication.ats_connectivity  a on 
